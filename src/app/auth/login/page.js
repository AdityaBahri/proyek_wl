    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', zIndex: 1 }}>
      <div className="glass slide-up" style={{ padding: 40, maxWidth: 440, width: '100%' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="input-group" style={{ marginBottom: 24 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPw ? 'text' : 'password'} className="glass-input" placeholder="Masukkan password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required style={{ paddingRight: 44 }} />
          ...
          <button type="submit" className="btn btn-primary" disabled={loading} style={{ width: '100%', padding: '14px', fontSize: '1rem' }}>
            {loading ? <span className="spinner" style={{ width: 20, height: 20, margin: 0, borderWidth: 2 }} /> : <><LogIn size={18} /> Masuk</>}
          </button>
        </form>
        ...
        <div style={{ marginTop: 24, padding: 16, background: 'rgba(108,99,255,0.05)', borderRadius: 12, border: '1px solid rgba(108,99,255,0.1)' }}>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: 8 }}>Demo Accounts:</p>
