...
import { formatCurrency, formatDateTime, formatBookingId, getStatusBadgeClass, getStatusLabel } from '@/lib/utils';
    ...
    <div className="fade-in">
      ...
      {loading ? <div className="spinner" /> : filteredData.length === 0 ? (
        <div className="glass empty-state">
          ...
          {filter !== 'all' && <button className="btn btn-secondary btn-sm" onClick={() => setFilter('all')} style={{ marginTop: 12 }}>Reset Filter</button>}
        </div>
      ) : (
        <div className="glass">
      ...
      {selectedBooking && (
        <div className="modal-overlay" onClick={() => !statusUpdating && setSelectedBooking(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Detail Pemesanan {formatBookingId(selectedBooking.id)}</h3>
              <button className="modal-close" onClick={() => !statusUpdating && setSelectedBooking(null)} disabled={statusUpdating}><X size={20} /></button>
            </div>
              ...
              {selectedBooking.payment ? (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                    ...
                    <div><span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>Status Pembayaran</span><div><span className={`badge ${getStatusBadgeClass(selectedBooking.payment.paymentStatus)}`}>{getStatusLabel(selectedBooking.payment.paymentStatus)}</span></div></div>
                  </div>
                  ...
                  {selectedBooking.payment.paymentStatus === 'pending' && (
                    <div style={{ display: 'flex', gap: 12 }}>
                      <button className={`btn btn-success btn-sm ${statusUpdating ? 'btn-loading' : ''}`} style={{ background: 'var(--success)' }} onClick={() => handleVerifyPayment(selectedBooking.payment.id, 'verified')} disabled={statusUpdating}>
                        {statusUpdating ? <span className="btn-spinner" /> : <><CheckCircle size={14} /> Verifikasi Pembayaran</>}
                      </button>
                      <button className={`btn btn-danger btn-sm ${statusUpdating ? 'btn-loading' : ''}`} onClick={() => handleVerifyPayment(selectedBooking.payment.id, 'rejected')} disabled={statusUpdating}>
                        {statusUpdating ? <span className="btn-spinner" /> : <><XCircle size={14} /> Tolak</>}
                      </button>
                    </div>
                  )}
                </>
            ...
            <div style={{ borderTop: '1px solid var(--glass-border)', paddingTop: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                {selectedBooking.status === 'paid' && (
                  <button className={`btn btn-primary ${statusUpdating ? 'btn-loading' : ''}`} onClick={() => handleUpdateStatus(selectedBooking.id, 'completed')} disabled={statusUpdating}>
                    {statusUpdating ? <span className="btn-spinner" /> : 'Tandai Selesai'}
                  </button>
                )}
                {selectedBooking.status !== 'cancelled' && selectedBooking.status !== 'completed' && (
                  <button className={`btn btn-danger ${statusUpdating ? 'btn-loading' : ''}`} onClick={() => handleUpdateStatus(selectedBooking.id, 'cancelled')} disabled={statusUpdating}>
                    {statusUpdating ? <span className="btn-spinner" /> : 'Batalkan Pemesanan'}
                  </button>
                )}
              </div>
            </div>
